...
fn hex_sampling_port_is_sampler_under_new_name() {
    fn _check_same<T: ?Sized>() {
        ...
        // `PhantomData<T>` is the standard idiom for "this generic parameter
        // is consumed by the type signature only"; without it clippy flags
        // `extra_unused_type_parameters` even though `T` is the whole point.
        let _phantom: PhantomData<T> = PhantomData;
    }
