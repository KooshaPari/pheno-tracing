...
| 2026-06-20 | orch-v12-s4-016 | L0 | infra | deny.toml,.github/workflows/deny.yml,.github/workflows/audit.yml,justfile | chore: cargo-deny dependency audit gate — deny.toml (advisories + bans + licenses + sources + features + imports + graph); .github/workflows/deny.yml + .github/workflows/audit.yml cargo-deny jobs with explicit --workspace flag; justfile `deny` recipe (`cargo deny check`); tier-0 hygiene per ADR-019 | macbook |
